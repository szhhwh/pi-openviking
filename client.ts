...
export class OVClient {
   ...
   *
   * The shared http layer owns the timeout controller; any caller-provided
   * signal (e.g. the pi agent abort signal wired to Esc) is attached to the
   * underlying fetch too, so a user abort cancels the in-flight request
   * immediately instead of waiting out the timer. On failure the envelope
   * distinguishes the two causes: `error.aborted` marks a caller-requested
   * abort, `error.timedOut` an internal timeout.
   */
  async fetchJSON<T>(
    path: string,
    init?: RequestInit,
    options?: OvHttpRequestOptions | { timeoutMs?: number } | number,
    signal?: AbortSignal,
  ): Promise<OVResponse<T>> {
    // Legacy call shape: fetchJSON(path, init, timeoutMs, signal).
    if (typeof options === "number") options = { timeoutMs: options };
    // The shared layer flags its own timeout as `{ aborted: true }` (it owns
    // the controller); with no external abort involved that is a timeout.
    const reclassifyTimeout = (res: OVResponse<T>): OVResponse<T> =>
      res.error?.aborted
        ? { ok: false, result: null, status: 0, error: { message: res.error.message || "timeout", aborted: false, timedOut: true } }
        : res;
    const signals: AbortSignal[] = [];
    if (signal) signals.push(signal);
    if (init?.signal) signals.push(init.signal);
    if (signals.length === 0) {
      return reclassifyTimeout(await this.http(path, init, options as OvHttpRequestOptions));
    }

    // The shared http layer owns the controller and overrides init.signal, so
    // an external abort cannot cancel the native fetch through init. Instead:
    // race the request against the signal for a prompt return, and track the
    // cause so the failure envelope distinguishes caller aborts from timeouts.
    const requestSignal = signals.length > 1 ? AbortSignal.any(signals) : signals[0];
    let abortedExternally = requestSignal.aborted;
    const onAbort = () => { abortedExternally = true; };
    requestSignal.addEventListener("abort", onAbort, { once: true });
    const httpPromise = this.http(path, init, options as OvHttpRequestOptions);
    // A request still in flight when the race is won must not crash the
    // process later as an unhandled rejection.
    httpPromise.catch(() => {});
    let winner: OVResponse<T> | null = null;
    ...
    if (winner === null) {
      return { ok: false, result: null, status: 0, error: { message: "aborted", aborted: true } };
    }
    // Mid-body aborts can surface as a resolved envelope; classify by cause.
    if (abortedExternally) {
      return { ok: false, result: null, status: 0, error: { message: "aborted while reading response body", aborted: true } };
    }
    return reclassifyTimeout(winner);
  }
  ...
  async abstract(uri: string, signal?: AbortSignal): Promise<string | null> {
    const res = await this.fetchJSON<string>(
      ...
      undefined, { timeoutMs: 10000 }, signal,
    );
    return res.ok ? res.result : null;
  }
